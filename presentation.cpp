...
void presentation(string path)
{
    ...
    while (true) {
        ...
        drawStatistics_PRESENTATION(iterator);
...
void drawAnimated_PRESENTATION(int iterator, vector<vector<Point2f>> awayPlayers_vector, vector<vector<Point2f>> homePlayers_vector, vector<Point2f> ball_vector, vector<vector<Point2f>> corners_vector, vector<vector<Point2f>> poolBoundaryCamera) {
    ...
    if (COVID) {
        ...
        for (int i=0; i<playerTail_vector[0].size(); i++) {
            ...
            else {circle(pool, Point2f(playerTail_vector[iterator][i].x*20,playerTail_vector[iterator][i].y*20), 20*1.5/2, playerTail_colors[i], FILLED);}
        }
    }
    
    // circle on each away player
    for (int i=0; i<awayPlayers_animated.size(); i++) {
        if (awayPlayers_animated[i] != Point2f(-10,-10)) {
            circle(pool, awayPlayers_animated[i], 8, away.primaryColor, FILLED);
            circle(pool, awayPlayers_animated[i], 9, away.secondaryColor, 2);
        }
    }
    if (COVID) {
        ...
        for (int i=0; i<awayExposure_vector[0].size(); i++) {
            if (awayExposure_vector[iterator][i] != 0) {
                ...
                circle(pool, awayPlayers_animated[i], 5, Xcolor, FILLED);
            }
        }
    }
    
    // circle on each home player
    for (int i=0; i<homePlayers_animated.size(); i++) {
        if (homePlayers_animated[i] != Point2f(-10,-10)) {
            circle(pool, homePlayers_animated[i], 8, home.primaryColor, FILLED);
            circle(pool, homePlayers_animated[i], 9, home.secondaryColor, 2);
        }
    }
    if (COVID) {
        // show a smaller circle on players that are in the danger zone
        ...
        for (int i=0; i<awayExposure_vector[0].size(); i++) {
            if (homeExposure_vector[iterator][i] != 0) {
                ...
                circle(pool, homePlayers_animated[i], 5, Xcolor, FILLED);
            }
        }
    }
    
    // circle on the ball
    if (ball_animated != Point2f(-10,-10)) {
        circle(pool, ball_animated, 5, ball.primaryColor, FILLED);
        circle(pool, ball_animated, 6, ball.secondaryColor, 2);
    }
